...
export const autodeskPlugin = new Elysia()
	.get(
		'/oauth2/autodesk/authorization',
	...
	.get(
		...
		async ({
			...
			try {
				const oauthResponse =
					await autodeskOAuth2Client.validateAuthorizationCode({
						code,
						codeVerifier
					});
				console.log('\nAutodesk authorized:', oauthResponse);
			} catch (err) {
	...
	.post(
		'/oauth2/autodesk/tokens',
		async ({ error, body: { refresh_token } }) => {
			try {
				const oauthResponse =
					await autodeskOAuth2Client.refreshAccessToken(
						refresh_token
					);
				console.log('\nAutodesk token refreshed:', oauthResponse);
	...
	.get(
		...
		async ({ error, headers: { authorization } }) => {
			...
			try {
				const userProfile =
					await autodeskOAuth2Client.fetchUserProfile(accessToken);
				console.log('\nAutodesk user profile:', userProfile);
