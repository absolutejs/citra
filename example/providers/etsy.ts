...
const etsyOAuth2Client = await createOAuth2Client('etsy', {
...
export const etsyPlugin = new Elysia()
	.get(
		...
		async ({ redirect, error, cookie: { state } }) => {
			...
			const authorizationUrl =
				await etsyOAuth2Client.createAuthorizationUrl({
					state: currentState
				});
	...
	.get(
		...
		async ({
			...
			try {
				const oauthResponse =
					await etsyOAuth2Client.validateAuthorizationCode({
						code
					});
				console.log('\nEtsy authorized:', oauthResponse);
			} catch (err) {
	...
	.post(
		...
		async ({ error, body: { refresh_token } }) => {
			try {
				const oauthResponse =
					await etsyOAuth2Client.refreshAccessToken(refresh_token);
				console.log('\nEtsy token refreshed:', oauthResponse);
	...
	.get(
		...
		async ({ error, headers: { authorization } }) => {
			...
			try {
				const userProfile =
					await etsyOAuth2Client.fetchUserProfile(accessToken);
				console.log('\nEtsy user profile:', userProfile);
