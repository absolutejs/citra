...
export const kakaoPlugin = new Elysia()
	.get(
		'/oauth2/kakao/authorization',
		async ({ redirect, error, cookie: { state, code_verifier } }) => {
			...
			const authorizationUrl =
				await kakaoOAuth2Client.createAuthorizationUrl({
					codeVerifier,
					state: currentState
				});
	...
	.get(
		...
		async ({
			...
			try {
				const oauthResponse =
					await kakaoOAuth2Client.validateAuthorizationCode({
						code,
						codeVerifier
					});
				console.log('\nKakao authorized:', oauthResponse);
			} catch (err) {
	...
	.post(
		...
		async ({ error, body: { refresh_token } }) => {
			try {
				const oauthResponse =
					await kakaoOAuth2Client.refreshAccessToken(refresh_token);
				console.log('\nKakao token refreshed:', oauthResponse);
	...
	.get(
		...
		async ({ error, headers: { authorization } }) => {
			...
			try {
				const userProfile =
					await kakaoOAuth2Client.fetchUserProfile(accessToken);
				console.log('\nKakao user profile:', userProfile);
