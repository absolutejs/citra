...
if (
	!env.TIK_TOK_CLIENT_KEY ||
	!env.TIK_TOK_CLIENT_SECRET ||
	!env.TIK_TOK_REDIRECT_URI
) {
	throw new Error('TikTok OAuth2 credentials are not set in .env file');
}

const tiktokOAuth2Client = await createOAuth2Client('tiktok', {
	clientId: env.TIK_TOK_CLIENT_KEY,
	clientSecret: env.TIK_TOK_CLIENT_SECRET,
	redirectUri: env.TIK_TOK_REDIRECT_URI
});

export const tiktokPlugin = new Elysia()
	.get(
		'/oauth2/tiktok/authorization',
		async ({ redirect, error, cookie: { state, code_verifier } }) => {
			...
			const authorizationUrl =
				await tiktokOAuth2Client.createAuthorizationUrl({
					codeVerifier,
					state: currentState
				});
	...
	.get(
		...
		async ({
			...
			try {
				const oauthResponse =
					await tiktokOAuth2Client.validateAuthorizationCode({
						code,
						codeVerifier
					});
				console.log('\nTikTok authorized:', oauthResponse);
			} catch (err) {
	...
	.post(
		'/oauth2/tiktok/tokens',
		async ({ error, body: { refresh_token } }) => {
			try {
				const oauthResponse =
					await tiktokOAuth2Client.refreshAccessToken(refresh_token);
				console.log('\nTikTok token refreshed:', oauthResponse);
	...
	.delete(
		...
		async ({ error, query: { token_to_revoke } }) => {
			...
			try {
				await tiktokOAuth2Client.revokeToken(token_to_revoke);
				console.log('\nTikTok token revoked:', token_to_revoke);
	...
	.get(
		...
		async ({ error, headers: { authorization } }) => {
			...
			try {
				const userProfile =
					await tiktokOAuth2Client.fetchUserProfile(accessToken);
				console.log('\nTikTok user profile:', userProfile);
